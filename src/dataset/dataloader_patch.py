...
class RealHDRDataset(Dataset):

    def __init__(self, hdr_prefix, is_training):
        self._hdr_prefix = os.path.join(hdr_prefix, 'HDR_gt/')
        self._ldr_prefix = os.path.join(hdr_prefix, 'LDR_in/')
        self.is_training = is_training
        self.val_start_idx = 8000
        self.num_samples = 9786
        return

    def __getitem__(self, idx):
        if self.is_training:
            chosen_idx = np.random.randint(idx, self.val_start_idx)
            return HDRDataset._hdr_read_resize(os.path.join(self._hdr_prefix, f'{chosen_idx:05}.hdr'), self.is_training)
        else:
            val_idx = self.val_start_idx + idx
            return HDRDataset._hdr_read_resize(os.path.join(self._hdr_prefix, f'{val_idx:05}.hdr'), self.is_training)
...
class PatchHDRDataset(Dataset):
    def __init__(self, hdr_prefix, hdr_posfix_list, is_training=True, n_way=7, load_to_mem=False):
        self.synth_hdr_dataset = HDRDataset(os.path.join(hdr_prefix, 'HDR-Synth/'), hdr_posfix_list, is_training)
        self.real_hdr_dataset = RealHDRDataset(os.path.join(hdr_prefix, 'HDR-Real/'), is_training)
        self.n_way = n_way

        crf_list = self._get_crf_list('/home/users/edwinpan/MetaHDR/dorfCurves.txt')
    ...
    def __getitem__(self, idx):
        ...
        if self._is_training:
            if np.random.randint(0,2):
                hdr = self.synth_hdr_dataset[idx // 2]
            else:
                hdr = self.real_hdr_dataset[idx // 2]
        else:
            hdr = self.real_hdr_dataset[idx // 2]
    ...
    def __len__(self):
        return 2 * (len(self.synth_hdr_dataset) + len(self.real_hdr_dataset))
